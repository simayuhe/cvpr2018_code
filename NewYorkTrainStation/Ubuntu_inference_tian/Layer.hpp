...
template<typename D> void Layer<D>::load_txt_trainss(string& trainss_file)//20170821
{
	...
	/*		char txtNo[10];
			...
			FILE *fileptr;
			fileptr = fopen(dir, "r");
			while (fscanf(fileptr, "%d", &word) != EOF)//?a????¨®¨²¡Á?????¦Ì¦Ì?¨¦¨°??€?¨¨?a¦Ì¨¤¨ºy¨¢?
			{
				trainss.push_back(word-1);//word ¨º??¨®1¦Ì?25¡ê?o¨®??¨ª3???¨º?¦Ì¦Ì?¨º¡Ào¨°¨°a?¨®0¨ª3??
				inds_groups.push_back(i);
				inds_items_i.push_back(num_words++);
				//printf("num_words ¡êo%d", word - 1);
			}
			fclose(fileptr);
		}
		//cin >> j;
	...
	else
	{
		...
		for (i = 0; i != num_groups; i++)
		{
			...
			for (j = 0; j != num_cands_i - 1; j++)
			{
				...
				group_candidates_i.push_back(word - 1);//???¨´¨®D¡À¨ºo?????¨¢?¨°???
			...
			group_candidates_i.push_back(i);//¨¦¨¨??¡Á?¨¢??¨®¦Ì??¦Ì¨º?alpha 1
			log_group_priors_i.push_back(log_alpha_group);
			group_priors_i.push_back(alpha_group);
			///cout << endl;
		}
