      ...
      for i = 1:nsamp
          ...
          elseif fine
              ...
              if det(covmat) < exp(-50*p)
                  while rcond(covmat) <= 1.405213e-16
                      alpha0 = 1e-6;
                      covmat  = (1-alpha0)*covmat + alpha0*(eps+trace(covmat)/size(covmat,1))*eye(size(covmat,1));     %Added by Guodong Tian on 1.8.2013
                  end
...
%                               meanvct(onesIDX,:))'.*eigvct(1,onesIDX)));
                          dist = abs(sum((data - ...
                              meanvct(onesIDX,:))'.*eigvct));       %modified by Guodong Tian
                          obsinplane = find(dist < 1e-8);
...
%                           z = z(:,onesIDX);%modified by Guodong Tian
                          dist = abs(sum((data - meanvct(onesIDX,:))'.*z));
                          madTMP = mad(ones(1,p),:);
                          % modified by Guodong Tian
                          thr_tmp = 1e-8;
                          inds_tmp = find(dist<thr_tmp);
                          while isempty(inds_tmp)
                              thr_tmp = 10*thr_tmp;
                              inds_tmp = find(dist<thr_tmp);
                          end
                          covar = cov(data(inds_tmp,:)).*madTMP.*madTMP';                          
